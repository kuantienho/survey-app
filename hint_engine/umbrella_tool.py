# scripts/run_umbrella_tool_v6_opposite_powered.py
# v6 umbrella tool (powered) — "opposite" additives mode + upgraded signal coverage
#
# Goal:
# - Keep your current umbrella v5 pipeline + opposite-gate behavior
# - Improve NEG signal capture (coverage) for common umbrella failure modes
# - Add tiny safety/quality upgrades WITHOUT killing cross-domain exploration:
#     1) NSFW hard-ban (prevents occasional garbage like "sex")
#     2) Low-info soft-penalty (e.g., "hair", "surface") — not banned, just de-prioritized
#     3) Weak-adjective cleanup in signals (avoid "strong/heavy/little" as standalone signals)
#
# Offline only (FAISS object pool + local CSV condition bank).
#
# Pipeline:
# 1) NEG signals (rule + fallback prototypes)  [UPGRADED coverage]
# 2) Condition bank expands NEG only (bonus) + sanity gate + drift filter
# 3) FAISS object pool retrieve using NEG query
# 4) Opposite gate: anti-neg + "solution-ish" anchors -> ranked pool + q_rank
# 5) MMR rerank using q_rank
# 6) EDGE filter (phrase-shape only, 1~3 tokens, POS/stopword/no-verb; no sim hard gate)
# 7) Output bans (umbrella components + human/pronoun + echo + text-domain + NSFW hard ban)
# 8) Final: top-9 closest-to-q_rank with light diversity caps
#
# CLI:
#   python scripts/run_umbrella_tool_v6_opposite_powered.py --text "..." [--debug-json]
#
# Notes:
# - Cross-domain outputs are allowed (razor+chocolate style).
# - "bone"/"blocks" style objects are allowed if they pass gates/bans.
# - We do NOT domain-gate the bank; we only sanity-gate phrase shape to avoid wiki-junk.
...
from paths import (
    UMBRELLA_OBJECTS_META,
    UMBRELLA_OBJECTS_FAISS,
    UMBRELLA_COND_BANK_META,
    UMBRELLA_COND_BANK_FAISS,
)
...
# Low-info soft penalty (NOT a ban)
LOW_INFO_PENALTY_GAMMA = 0.08  # small: just de-prioritize


# =========================
# "solution-ish" anchors
# =========================
_SOLUTION_ANCHORS = [
    # sealing / waterproofing
    "seal", "gasket", "o-ring", "valve", "vent", "pressure relief",
    "membrane", "barrier", "liner", "patch", "tape", "adhesive", "caulk",
    "hydrophobic coating", "waterproof coating",
    # structure / stiffening
    "stiffener", "rib", "brace", "strut", "spar", "frame", "skeleton",
    "reinforcement", "reinforced", "composite", "fiberglass", "carbon fiber",
    # locking / latching
    "latch", "detent", "ratchet", "lock", "clip", "clamp", "fastener",
    "hook", "loop", "velcro", "strap", "cord", "buckle", "toggle",
    "spring", "torsion spring",
    # flow / runoff control
    "drain", "channel", "nozzle", "lip", "edge", "skirt", "deflector",
    # repair / cleanup affordances
    "tool", "driver", "screw", "pin", "rivet", "replacement", "kit",
]
...
def _find_data_dir(start_dir: str) -> Tuple[str, str]:
    ...
    layout:
      - "website": <root>/data/umbrella
      - "proto":   <root>/data
    """
    cur = os.path.abspath(start_dir)
    for _ in range(7):
        cand_dom = os.path.join(cur, "data", "umbrella")
...
# =========================
# Umbrella component + substitute bans (output-side)
# =========================
UMBRELLA_COMPONENT_BAN_HARD = {
    "umbrella", "umbrellas",
    "canopy", "fabric",
    "rib", "ribs", "spoke", "spokes", "stretcher", "stretchers",
    "shaft", "handle", "grip", "tip", "tips", "runner", "ferrule",
    "latch", "button", "spring", "hinge",
}

UMBRELLA_SUBSTITUTE_BAN_SOFT = {
    "raincoat", "poncho", "jacket", "hood", "cape",
    "parasol", "sunshade",
}

def _is_umbrella_component_or_core(s: str) -> bool:
    s0 = _norm(s)
    if not s0:
        return False
    toks = s0.split()
    if not toks:
        return False
    if len(toks) == 1 and toks[0] in UMBRELLA_COMPONENT_BAN_HARD:
        return True
    if any(t in UMBRELLA_COMPONENT_BAN_HARD for t in toks):
        return True
    return False

def _is_umbrella_substitute_soft(s: str) -> bool:
    s0 = _norm(s)
    for w in UMBRELLA_SUBSTITUTE_BAN_SOFT:
...
# =========================
# Two-layer TEXT DOMAIN ban (same family as pen)
# =========================
TEXT_DOMAIN_TOKEN_BAN = {
    ...
    "name","names", "machine", "machines", "damage", "pressure"
}
...
# =========================
# NSFW hard ban (tiny, strict)
# =========================
_NSFW_TOKENS = {
    "sex", "porn", "porno", "nude", "naked", "fetish", "erotic", "xxx"
}

def _nsfw_hit(s: str) -> bool:
    toks = _norm(s).split()
    return any(t in _NSFW_TOKENS for t in toks)
...
# =========================
_LOW_INFO_UNIGRAMS = {
    "hair", "surface", "liquid", "breath", "air", "water", "thing", "stuff"
}
def _low_info_penalty(phrase: str) -> float:
    p = _norm(phrase)
    if not p:
        return 0.0
    toks = p.split()
    # penalize mainly unigrams (or phrases that end with one)
    if len(toks) == 1 and toks[0] in _LOW_INFO_UNIGRAMS:
        return LOW_INFO_PENALTY_GAMMA
    # tiny penalty if phrase is extremely generic
    if len(toks) == 2 and toks[-1] in _LOW_INFO_UNIGRAMS:
        return LOW_INFO_PENALTY_GAMMA * 0.6
    return 0.0


# =========================
# Negative signal extraction (rule + fallback)  [POWERED coverage]
# =========================
GENERIC_ADJ_STOP = {
    "new","old","other","same","different","first","last","many","few","most","some",
    "large","small","big","high","low","long","short",
    # extra weak/low-value for umbrella signals (still allow as part of noun chunks)
    "strong","heavy","little","slight","minor","major",
}

# expanded cues: invert/inside-out, latch/lock failures, leaks/seep/drip, tears, bent ribs, stuck open/close
ISSUE_VERB_CUE = re.compile(
    r"\b("
    r"inside\s*out|turns?\s+inside\s*out|flips?\s+inside\s*out|inverts?|invert(ed|s)?|"
    r"flip|flips|flipped|"
    r"leak|leaks|leaking|leakage|seep|seeps|seeping|"
    r"drip|drips|dripping|"
    r"soak|soaks|soaking|"
    r"pool|pools|pooling|"
    r"tear|tears|torn|rip|rips|ripping|"
    r"puncture|punctures|punctured|"
    r"break|breaks|broken|snap|snaps|snapped|"
    r"bend|bends|bent|warp|warps|warped|"
    r"rust|rusted|corrode|corrodes|corroded|"
    r"jam|jams|jammed|stuck|sticks|sticking|"
    r"won['’]?t\s+open|doesn['’]?t\s+open|won['’]?t\s+close|doesn['’]?t\s+close|"
    r"won['’]?t\s+lock|doesn['’]?t\s+lock|won['’]?t\s+latch|doesn['’]?t\s+latch|"
...
PHYS_PATTERNS = [
    ...
    re.compile(r"\b(turns?\s+inside\s+out|inside\s+out)\b", re.I),
    re.compile(r"\b(won['’]?t\s+lock|doesn['’]?t\s+lock|lock\s+won['’]?t)\b", re.I),
    re.compile(r"\b(won['’]?t\s+open|doesn['’]?t\s+open|won['’]?t\s+close|doesn['’]?t\s+close)\b", re.I),
    re.compile(r"\b(heavy\s+rain|strong\s+wind|wind\s+gusts?|gusty\s+wind)\b", re.I),
]

# prototypes for fallback when signals are too few
_COMPLAINT_PROTOTYPES = [
    "umbrella turns inside out in strong wind",
    "umbrella flips inside out in gusts",
    "latch won't lock",
    "button won't lock",
    "umbrella leaks during heavy rain",
    "water seeps through canopy",
    "umbrella drips at the seams",
    "ribs bend or snap",
    "umbrella won't open",
    "umbrella won't close",
    "runner gets stuck",
    "fabric tears near the ribs",
    "rusted hinge jams",
]
...
def extract_negative_signals_rule(text: str, max_signals: int = 25) -> List[str]:
    ...
    # (1) adjectives (skip generic + weak)
    for tok in doc:
        if tok.pos_ == "ADJ":
            w = tok.lemma_.lower()
    ...
    # (3) noun chunks that look issue-ish
    issue_kw = (
        r"(umbrella|canopy|rib|ribs|shaft|handle|runner|stretcher|"
        r"wind|gust|storm|rain|"
        r"inside\s*out|invert|flip|"
        r"leak|seep|drip|soak|pool|"
        r"tear|rip|puncture|"
        r"bend|bent|snap|broken|warp|"
        r"rust|corrode|"
        r"lock|latch|button|jam|stuck|open|close|collapse|slip)"
    )
    for chunk in doc.noun_chunks:
        ...
        if has_adj or has_issue_word:
            # also drop super-weak unigrams like "strong" if chunk is only that
            if len(t.split()) == 1 and t in {"strong", "heavy", "little", "slight"}:
                continue
            signals.append(t)
...
_RES: Optional[UmbrellaResources] = None
...
def load_umbrella_resources(model_name: str = MODEL_NAME_DEFAULT) -> UmbrellaResources:
    global _RES
    if _RES is not None:
        return _RES

    if not os.path.exists(UMBRELLA_OBJECTS_META):
        raise FileNotFoundError(f"Missing: {UMBRELLA_OBJECTS_META}")
    if not os.path.exists(UMBRELLA_OBJECTS_FAISS):
        raise FileNotFoundError(f"Missing: {UMBRELLA_OBJECTS_FAISS}")

    obj_df = pd.read_csv(UMBRELLA_OBJECTS_META)
    if "object" in obj_df.columns:
        ...
        raise ValueError("umbrella_objects_meta.csv must contain column 'object' or 'term'")
    obj_index = faiss.read_index(UMBRELLA_OBJECTS_FAISS)

    # use indexed condition bank (meta + faiss)
    if not os.path.exists(UMBRELLA_COND_BANK_META):
        raise FileNotFoundError(f"Missing: {UMBRELLA_COND_BANK_META}")
    if not os.path.exists(UMBRELLA_COND_BANK_FAISS):
        raise FileNotFoundError(f"Missing: {UMBRELLA_COND_BANK_FAISS}")
...
def expand_conditions_with_bank_bonus(
    ...
    gate_dbg = {
        ...
        "bank_csv": UMBRELLA_COND_BANK_META,
        "bank_faiss": UMBRELLA_COND_BANK_FAISS,
        "retrieved_n": 0,
        "returned_n": 0,
...
EDGE_UNIGRAM_BAN = {
    ...
    "word","words","keyword","keywords","usage","phenomena",
...
# =========================
# Final selection: closest with diversity caps
# =========================
def _score_to_query(cands: List[str], q: np.ndarray, model: SentenceTransformer) -> List[Tuple[str, float]]:
    ...
    # apply soft penalty AFTER similarity
    pairs = []
    for c, s in zip(cands, sims):
        s2 = float(s) - float(_low_info_penalty(c))
        pairs.append((c, s2))
    pairs.sort(key=lambda x: x[1], reverse=True)
    return pairs
...
def _ban_candidate(
    ...
    if _is_umbrella_component_or_core(c0):
        return True, "umbrella_component"

    if enable_soft_substitute_ban and _is_umbrella_substitute_soft(c0):
        return True, "umbrella_substitute"
...
def opposite_rank_pool(
    ...
    # score and rank (apply low-info penalty here too)
    score = float(a_sol) * sim_sol - float(b_neg) * sim_neg
    score2 = score[idx2]

    # soft penalty: subtract gamma for low-info candidates
    penalties = np.asarray([_low_info_penalty(pool[i]) for i in idx2], dtype="float32")
    score2 = score2 - penalties
...
# Main API
# =========================
def get_umbrella_add(
    text: str,
    debug_json: bool = False,
    opposite_enabled: bool = OPPOSITE_ENABLED_DEFAULT,
) -> Dict[str, Any]:
    res = load_umbrella_resources(model_name=MODEL_NAME_DEFAULT)
    model = get_model(res.model_name)
    ...
    if not neg_phrases:
        ...
        if debug_json:
            out["debug"] = {
                ...
                "bank_name": res.bank_name,
                "opposite_gate": {"enabled": bool(opposite_enabled), "used": False},
                ...
                "paths_resolved": {
                    "objects_meta": UMBRELLA_OBJECTS_META,
                    "objects_faiss": UMBRELLA_OBJECTS_FAISS,
                    "cond_bank_csv": UMBRELLA_COND_BANK_META,
                    "cond_bank_faiss": UMBRELLA_COND_BANK_FAISS,
                },
            }
        return out
    ...
    if opposite_enabled:
        ...
        opposite_dbg = {
            ...
            "low_info_penalty_gamma": LOW_INFO_PENALTY_GAMMA,
        }
    ...
    ban_dropped: Dict[str, int] = {
        "umbrella_component": 0,
        "umbrella_substitute": 0,
        "echo": 0,
        "bad_start": 0,
        "human_role": 0,
        "empty": 0,
    ...
    if debug_json:
        out["debug"] = {
            ...
            "policy": {
                ...
                "max_per_head": MAX_PER_HEAD_DEFAULT,
                "bank_policy": "bank expands NEG only (bonus); NO domain gate; YES sanity gate; used if not drifted and kept>=min_bank_to_use; cap=bank_seed_cap",
                "bank_sanity": {
                ...
                "opposite_policy": {
                    ...
                    "anchors": "mechanism/repair oriented list (cross-domain allowed)",
                ...
                "signal_policy": "powered coverage + weak-adjective cleanup",
                "nsfw_policy": "hard-ban tiny token set",
                "low_info_policy": "soft penalty only (NOT ban)",
            }
        }
...
def main():
    ...
    ap.add_argument("--text", required=True, help="Describe umbrella issues (English).")
